...
public class ArticleController {
    ...
    public String write(ArticleDTO articleDTO, HttpServletRequest req){
        ...
        articleDTO.setReg_ip(req.getRemoteAddr());
        ...
        //글저장
        articleDTO.setFile_cnt(fileDTOList.size());
        int ano = articleService.save(articleDTO);

        log.info("ArticleController ano ={}", ano);

        //파일 저장
        for(FileDTO fileDTO : fileDTOList){
            System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@@@@");
            fileDTO.setAno(ano);
            log.info("Article Controller file save fileDTO={}", fileDTO);
